//Source: https://github.com/adam-singer/dart-sorting-algorithms/blob/master/src/QuickSort/QuickSort.dart
...
class QuickSort {
  Map<Restaurant, double> scoredRestaurants;
  static void sortWithHashmap(List<String> a, Map<Restaurant, double> sR) {
  ...
  static void _sortWithComparable(List<Comparable> a, double lo, double hi) {
    if (hi <= lo) {
      return;
    }

    double j = _partition(a, lo, hi);
  ...
  static double _partition(List<Comparable> a, double lo, double hi) {
    double i = lo;
    double j = hi + 1;
    Comparable v = a[lo];
...
//      double r = i + (random() * (N - i)).todouble();
//      String swap = a[r];
//      a[r] = a[i];
//      a[i] = swap;
//    }
   ...
   ***********************************************************************/
  static Comparable select(List<Comparable> a, double k) {
    if (k < 0 || k >= a.length) {
      throw new Exception("Selected element out of bounds");
    }

    _shuffle(a);
    double lo = 0, hi = a.length - 1;
    while (hi > lo) {
      double i = _partition(a, lo, hi);
      if (i > k) {
        hi = i - 1;
      } else if (i < k) {
        lo = i + 1;
  ...
  static void _exch(List a, double i, double j) {
    var swap = a[i];
    a[i] = a[j];
    a[j] = swap;
  }
  ...
  static bool _isSortedInRange(List<Comparable> a, double lo, double hi) {
    for (double i= lo + 1; i <= hi; i++) {
      if (_less(a[i], a[i-1])) {
        return false;
      }
    }
